...
\section{Definiciones y teoremas de especializacion}
...
\end{array}
\]
donde (casi todas) las fibras son curvas elipticas entonces podemos sumar e invertir sobre ellas, y elemento neutro $\sigma_\OO$, es el grupo abeliano de secciones de la superficie eliptica.
\end{definition}

Dos cosas que querriamos ahora son un analogo al teorema de Mordell-Weil, es decir que este grupo sea finitamente generado, y poder distinguir los casos triviales de los interesantes. Esto ultimo se refiere a que uno podria tener una curva eliptica definida sobre $K$ y decir que está definida sobre $K(\cc)$, o tambien tenerla sobre $K(\cc)$ pero que sea isomorfa a una definida sobre $K$, y ambos casos querriamos evitarlos para que la superficie realmente parametrice una familia de curvas elipticas distintas. Es facil ver que esta condicion es equivalente a pedir que la curva eliptica asociada no sea de la forma $E_0 \times \cc$ para una curva eliptica $E_0$ definida sobre el cuerpo $K$, ya que en este caso todas las fibras serian $E_0$. Resulta que estos dos elementos vienen de la mano:

\begin{theorem}[Mordell-Weil para cuerpos de funciones]
Sea $\ee\to\cc$ superficie eliptica definida sobre un cuerpo $K$ y $E$ la curva eliptica correspondiente definida sobre $K(\cc)$. Si $E$ no es isomorfa a $E_0 \times \cc$ entonces $\ee(\cc)$ es finitamente generado.
\end{theorem}

Este teorema corresponde al 6.1 del capitulo 3 del libro de Silverman~\cite{SilvermanAdvanced}, y su demostracion se puede encontrar ahi principalmente leyendo la seccion anterior.

pag 242

Remark 6.2.2. If k is a number field, or more generally if k is a finitely
generated extension of Q, then the Mordell-Weil theorem (6.1) is true regardless of whether or not $\ee$ → C splits. This generalization of the original
Mordell-Weil theorem for number fields is due to Néron; see exercise 3.4. (pag 290)
...
\section{Reduccion y fibras singulares}
